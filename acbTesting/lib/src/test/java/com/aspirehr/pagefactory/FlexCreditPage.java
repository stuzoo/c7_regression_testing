...
public class FlexCreditPage extends DetailPage {
	WebDriver driver;
	private static final String REGEX_PLAN_DETAILS = "__text\\d+-cont\\d+_\\d+";//__text170-cont0_1

	public FlexCreditPage(WebDriver driver) {
		super(driver);
		this.driver = driver;
...
//	public ArrayList<String> getFlexCreditDetails() {
//		List<WebElement> FlexCreditForm = SeleniumUtil.findElementListByClassName(this.driver, "sapUiFormResGridCont");
//		ArrayList<String> flexCreditDtsList = new ArrayList<String>();
//		for (WebElement element : FlexCreditForm) {
//			List<WebElement> flexCreditDetails = element.findElements(By.className("sapUiRespGridSpanL6"));
//			for (WebElement formFields : flexCreditDetails) {
//				if (formFields.getText() != null && !formFields.getText().trim().equals("")) {
//					flexCreditDtsList.add(formFields.getText());
//				}
//			}
//		}
//		return flexCreditDtsList;
//	}
...
//	}
	...
	// .until(ExpectedConditions.elementToBeClickable(flexEdit));
	// flexEdit.click();
	// }
	...
	public float getSelectedPlanCost(int planNo) {
		...
		planNo = planNo - 1;
		String fldSuffix = "-idCRED-" + planNo + "_cell4";
		List<WebElement> elementList = SeleniumUtil.findElementListByIdSuffix(driver, By.className("sapMListTblCell"),
				fldSuffix, 2);
