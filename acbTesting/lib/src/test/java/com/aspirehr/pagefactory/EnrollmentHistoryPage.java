...
public class EnrollmentHistoryPage extends DetailPage {
	WebDriver driver;
	...
	public EnrollmentHistoryPage(WebDriver driver) {
		super(driver);
		this.driver = driver;
	...
	// public ArrayList<String> getPlanDetails() {
	// List<WebElement> planForm =
	// SeleniumUtil.findElementListByClassName(this.driver, "sapUiRFLContainer");
	// ArrayList<String> planDtsList = new ArrayList<String>();
	// for (WebElement element : planForm) {
	// List<WebElement> planDetails =
	// element.findElements(By.className("sapUiRFLContainerContent"));
	// for (WebElement formFields : planDetails) {
	...
	public ArrayList<String> getToolbarButtons() {
		ArrayList<String> toolBarBtnList = new ArrayList<String>();
		WebElement currentBtnTxt = this.driver.findElement(By.id("currIcon"));
		WebElement pastBtnTxt = this.driver.findElement(By.id("histIcon"));
		WebElement futureBtnTxt = this.driver.findElement(By.id("futIcon"));
		WebElement pendingEOIBtnTxt = this.driver.findElement(By.id("eoiIcon"));
		WebElement expiredEOIBtnTxt = this.driver.findElement(By.id("eoiexIcon"));
		// WebElement resetBtnElement = SeleniumUtil.findElementByLinkText(driver,
		// "Reset");

		toolBarBtnList.add(currentBtnTxt.getAttribute("title"));
		toolBarBtnList.add(pastBtnTxt.getAttribute("aria-label"));
		toolBarBtnList.add(futureBtnTxt.getAttribute("title"));
		toolBarBtnList.add(pendingEOIBtnTxt.getAttribute("title"));
		toolBarBtnList.add(expiredEOIBtnTxt.getAttribute("title"));
		// toolBarBtnList.add(resetBtnElement.getText());
		return toolBarBtnList;

	}

	public ArrayList<String> getRecordStatuses() {
		ArrayList<String> recStatuses = new ArrayList<String>();
		Pattern regEx = Pattern.compile(REGEX_REC_STATUS_COLUMN);
		List<WebElement> statusColumn = SeleniumUtil.findElementListByIdRegex(driver, By.className("sapUiIcon"), regEx);
		for (WebElement element : statusColumn) {
			if (element.getAttribute("aria-label") != null && !element.getAttribute("aria-label").trim().equals("")) {
				if (element.getAttribute("aria-label").equalsIgnoreCase("quality-issue"))
					recStatuses.add(element.getAttribute("style").substring(24, 35));
				else
				recStatuses.add(element.getAttribute("aria-label"));				
			}
		}
		return recStatuses;
	}

	public void clickToolBarLink(String linkName) {
		switch (linkName) {
		case "current":
			WebElement currentBtn = this.driver.findElement(By.id("currIcon"));
			currentBtn.click();
			break;
		case "future":
			WebElement futureBtn = this.driver.findElement(By.id("futIcon"));
			futureBtn.click();
			break;
		case "past":
			WebElement pastBtn = this.driver.findElement(By.id("histIcon"));
			pastBtn.click();
			break;
		case "pendingeoi":
			WebElement pendingEOIBtn = this.driver.findElement(By.id("eoiIcon"));
			pendingEOIBtn.click();
			break;
		case "expiredeoi":
			WebElement expiredEOIBtn = this.driver.findElement(By.id("eoiexIcon"));
			expiredEOIBtn.click();
			break;
		case "reset":
			WebElement resetBtn = this.driver.findElement(By.id("__icon0"));
			resetBtn.click();
			break;
		}
	...
	public int getSelectedDeptCount() {
		...
		try {
			...
			List<WebElement> deptList = SeleniumUtil.findElementListByIdRegex(driver, By.tagName("input"), regEx, 2);
